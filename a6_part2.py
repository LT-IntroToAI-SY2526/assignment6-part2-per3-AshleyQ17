...
def visualize_features(data):
    ...
    #       - labels and title
    #       - grid
    axes[0, 0].scatter(data['Square Feet'], date['Price'], color='blue', alpha=0.6)
    ...
    axes[0, 1].set_xlabel('Bedroms(top right)')
    ...
    axes[1, 0].set_xlabel('Bathrooms(bottom left)')
    ...
    axes[1, 1].set_xlabel('Age(bottom right)')
...
def make_prediction(model, sqft, bedrooms, bathrooms, age):
    ...
    print(f"\n=== New Prediction ===")
    print(f"House specs: {sqft:.0f}sq ft, {bedrooms} number of bedrooms, {bathrooms} number of bathrooms, {age} years old")
